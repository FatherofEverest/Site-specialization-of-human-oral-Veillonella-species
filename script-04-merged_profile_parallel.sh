...
then
/workspace/jmarkwelchlab/P_1221_Veillonella/SCRIPTS/script-merged_profile-HP.sh ${projectID} ${site} &
fi

# make merged profile for 8 oral sites
num_processes=8

declare -a siteArray=("BM" "TD" "SUPP" "PT" "TH" "KG" "SUBP" "SV")
...
do
	...
	/workspace/jmarkwelchlab/P_1221_Veillonella/SCRIPTS/script-merged_profile.sh ${projectID} ${site} &
	fi
done
